...
parser.add_argument('--gpu', default=0, type=int,
                    help='GPU number')
parser.add_argument('--start-epoch', default=18, type=int, metavar='N',
...
class networkFunctions():
    ...
    def create_network_functions(self, network):
        ...
        def train_function(Xs, Ys, hist_boxes, hist_boxes_gt, loss_weights, network):
            ...
            m_1 = nn.CrossEntropyLoss(reduction='mean', weight=alpha1)
            m_2 = nn.CrossEntropyLoss(reduction='mean', weight=alpha2)
            m_3 = nn.CrossEntropyLoss(reduction='mean', weight=alpha3)
            m_4 = nn.CrossEntropyLoss(reduction='mean', weight=alpha4)
    ...
    if 'scale_4' in net.name:
        ...
        for layer in cfg.keys():
            ...
            for name, module in net.named_children():
                if layer == name and (not layer.startswith("conv_middle_")) and (
                        not layer.startswith("conv_lowest_") and (not layer.startswith("conv_scale1_"))):
                    lyr = module
                    ...
                    layer_copy_count += 1
                elif (layer.startswith("conv_middle_") or layer.startswith("conv_lowest_") or layer.startswith(
                        "conv_scale1_")) and name == layer:
                    lyr = module
...
def get_offset_error(x_pred, y_pred, x_true, y_true, output_downscale, max_dist=16):
    ...
    recall = np.asarray(num_hits, dtype='float32') / (
                np.asarray(num_hits, dtype='float32') + np.asarray(fn, dtype='float32'))
...
def get_boxed_img(image, h_map, w_map, gt_pred_map, prediction_downscale, thickness=1, multi_colours=False):
    ...
    for y, x in zip(Y, X):
        ...
        if h // 2 in BOXES[3] or h // 2 in BOXES[2]:
            ...
            t = thickness
        cv2.rectangle(boxed_img,
                      (max(int(prediction_downscale * x - w / 2), 0), max(int(prediction_downscale * y - h / 2), 0)),
                      (min(int(prediction_downscale * x + w - w / 2), W),
                       min(int(prediction_downscale * y + h - h / 2), H)), selected_colour, t)
...
def train_networks(network, dataset, network_functions, log_path):
    ...
    for e_i, epoch in enumerate(range(start_epoch, num_epochs)):
        ...
        # b_i - batch index
        for b_i in tqdm(range(num_batches_per_epoch)):
...
def train():
    ...
    if not dataset.dataset_ready:
        ...
        if args.dataset == "ucfqnrf" or args.dataset == "nwpu":
...
if __name__ == '__main__':
    ...
    # wsking
        ...
        dataset_paths = {'test': [r'/home/ubuntu/dataset/nwpu/images',
                                  r'/home/ubuntu/dataset/nwpu/jsons'],
                         'train': [r'/home/ubuntu/dataset/nwpu/images',
                                   r'/home/ubuntu/dataset/nwpu/jsons']}
        validation_set = 500
        output_downscale = 2
        path = '../dataset/nwpu_dotmaps_predictionScale_' + str(output_downscale)
