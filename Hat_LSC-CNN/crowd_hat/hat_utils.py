...
def load_crowd_hat(count_model_path=None, nms_model_path=None):
    try:
        from count_decoder import CountDecoder
        count_model = CountDecoder()
        count_model.load_state_dict(
            torch.load(count_model_path, map_location='cpu'))
        count_model.cuda().eval()
        from nms_decoder import NMSDecoder
        nms_model = NMSDecoder()
        nms_model.load_state_dict(
            torch.load(os.path.join(nms_model_path), map_location='cpu')
        )
        nms_model.cuda().eval()
    ...
    cfg.global_dic['nms_model'] = nms_model
