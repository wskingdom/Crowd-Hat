...
class Trainset(Dataset):
    def __init__(self, json_root, mode="train"):
        ...
        id_std[59] = 3098
        # val_list = ['nwpu_' + str(idx) + '_0.json' for idx in id_std]
        val_list = [name for name in os.listdir(json_root) if int(name.split('.')[0].split('_')[1]) in id_std
                    and name.split('.')[0].split('_')[-1] == '0']
        # val_list = [name for name in os.listdir(json_root) if 'nwpu' in name
        ...
        for path in tqdm(json_list):
            ...
            with open(path, 'r')as data:
                data = json.load(data)
            features_2d.append([data['feature2d'][-1]])
            features_1d.append([data['feature1d'][0]])
        ...
        # self.features_2d = self.features_2d[:, -3:, :, :]
        # self.features_1d = self.features_1d[:, 0, :].unsqueeze(dim=1)
...
def train_count_decoder(n, epochs, resume=0):
    """
    The training of count decoder can be split into 3 stages:
    stage1 : train the 1D convolution layers
    stage2 : train the 2D convolution layers
    stage3 : fix the weights of 1D and 2D convolution layers, and train a count mlp to regress the crowd count.
    @param n: times of training
    @param epochs: default training epochs in each training
    @param resume: resume from a training stage. e.g. resume = 0 means the next training stage is stage1
    """
    assert resume in [0, 1, 2]
    if not os.path.exists(cfg.weights_root):
        os.mkdir(cfg.weights_root)
    val_dataset = Trainset(cfg.training_data_root, "val")
        ...
        for i in range(n):
            ...
            for epoch in range(epochs):
                ...
                if min_loss > mean:
                    ...
                    torch.save(model.state_dict(), os.path.join(cfg.weights_root, 'count_decoder_%s_' % mode_idx + format(min_loss, '.4f') + '.pth'))
...
if __name__ == '__main__':
    train_count_decoder(5, 100, resume=0)
